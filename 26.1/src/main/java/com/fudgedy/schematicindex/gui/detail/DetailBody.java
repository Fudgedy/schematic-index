...
class DetailBody
{
	...
	void render(GuiGraphicsExtractor ctx, SchematicEntry entry, int mouseX, int mouseY, DetailView.Layout layout)
	{
	...
	private void tab(GuiGraphicsExtractor ctx, Rect rect, String label, boolean active, int mouseX, int mouseY)
	{
	...
	private void materials(GuiGraphicsExtractor ctx, SchematicEntry entry, int px, int top, int wide,
			int viewport, int lineStep)
	{
		...
		for (MaterialRow row : rows)
		{
			if (rowY + rowStep >= top && rowY <= top + viewport)
			{
				...
				if (row.icon() != null)
				{
					ctx.item(row.icon(), px, rowY + (rowStep - iconSize) / 2);
				}
