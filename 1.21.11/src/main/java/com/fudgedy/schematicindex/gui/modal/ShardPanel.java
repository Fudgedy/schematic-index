...
public class ShardPanel
{
	...
	private int renderReferral(GuiGraphics ctx, Font font, List<String> invite, int x, int y, int width,
	...
	private void renderFloaters(GuiGraphics ctx, Font font)
	{
	...
	private int renderDays(GuiGraphics ctx, Font font, int x, int y, int width, int mouseX, int mouseY)
	{
	...
	private void renderDayCard(GuiGraphics ctx, Font font, Shards.Day day, int cx, int y, int cardWidth)
	{
		...
		{
			ctx.renderItem(CHEST, iconX, y + 24);
		}
		else
		{
	...
	private int renderQuests(GuiGraphics ctx, Font font, List<Shards.Quest> quests, int x, int y,
			int width, int mouseX, int mouseY)
	{
		...
		for (Shards.Quest quest : quests)
		{
			...
			ctx.renderItem(this.questIcon(quest.icon()), x + 8, rowMid - 8);
	...
	private void renderBalance(GuiGraphics ctx, Font font, int x, int y, int width)
	{
