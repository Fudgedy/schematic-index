...
public final class MapCorners implements IRenderer
{
	...
	// MaLiLib on this version hands the world-last hook the camera itself rather than its render state
	@Override
	public void onRenderWorldLastAdvanced(RenderTarget fb, Matrix4f modelViewMatrix, Matrix4f projectionMatrix,
			Frustum culling, Camera camera, RenderBuffers buffers, ProfilerFiller profiler)
	{
		...
		float y = (float) (clampedHeight(mc.level) + HEIGHT_OFFSET - eye.y);
		...
		RenderContext ctx = new RenderContext(NAME, MaLiLibPipelines.POSITION_COLOR_TRANSLUCENT_NO_DEPTH_NO_CULL);
		BufferBuilder buffer = ctx.getBuilder();

		for (int column = minColumn; column <= maxColumn; column++)
		{
			float x = (float) (column * MAP_SIZE + EDGE_OFFSET - eye.x);

			for (int row = minRow; row <= maxRow; row++)
			{
				float z = (float) (row * MAP_SIZE + EDGE_OFFSET - eye.z);
